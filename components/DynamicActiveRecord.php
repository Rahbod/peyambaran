...
//    }
    ...
    {
        if (key_exists($name, $this->_dynamicAttributes)) {
            $path = explode('.', $name);
    ...
    public static function populateRecord($record, $row)
    {
        ...
        if (isset($row[$dynCol])) {
            ...
            foreach (array_keys(static::dynColDecode($row[$dynCol])) as $attribute)
                if (property_exists($record, $attribute))
                    $record->$attribute = $record->_dynamicAttributes[$attribute];
        } elseif (!empty($keys = preg_grep('/#rhb#.*/', array_keys($row)))) {
            foreach ($keys as $key) {
                list($attributeName, $subAttributeName) = explode('_', substr($key, 5));
                if (!isset($record->_dynamicAttributes[$attributeName]))
                    $record->_dynamicAttributes[$attributeName] = new \stdClass();

                if (is_array($row[$key]))
                    $record->_dynamicAttributes[$attributeName]->$subAttributeName = (object)$row[$key];
                else
                    $record->_dynamicAttributes[$attributeName]->$subAttributeName = $row[$key];
            }
        }
    ...
    /**
     * Return all extra field contents
     * @return mixed|null
     */
    public function getExtras()
    {
        return $this->extra ? Json::decode($this->extra) : null;
    }

    /**
     * Return specific index of extra field array
     *
     * @param $key
     * @return null
     */
     ...
     * @return string
     */
    public static function columnGetString($field, $alias = '', $as = false)
    {
        ...
        if ($as)
            ...
            return "COLUMN_GET({$alias}`{$dynamicFieldName}`, '{$field}' AS {$dynamicDefaultValues[$field][0]})";
    }
     ...
     * @return string
     */
    public static function columnJsonString($alias = '', $as = false)
    {
        $modelObject = Yii::createObject(self::className());
        $dynamicFieldName = $modelObject::dynamicColumn();
        $alias = $alias ? "`$alias`." : '';
        if ($as)
            return "COLUMN_JSON({$alias}`{$dynamicFieldName}`) AS {$as}";
        else
            return "COLUMN_JSON({$alias}`{$dynamicFieldName}`)";
    }
     ...
     */
    public static function columnExistsString($field, $alias = '')
    {
        $modelObject = Yii::createObject(self::className());
        $dynamicFieldName = $modelObject::dynamicColumn();
        $alias = $alias ? "$alias." : '';
        return "COLUMN_EXISTS({$alias}`{$dynamicFieldName}`, '{$field}')";
    }

    /**
     * Returns dynamic attribute values.
     * @return array dynamic attribute values (name => value).
     */
